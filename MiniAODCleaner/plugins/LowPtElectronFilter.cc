  ...
  //trk_(consumes<reco::GsfTrackCollection>(iConfig.getParameter<edm::InputTag>("Tracks")))
{
  ...
  produces<pat::ElectronCollection>( "MiniLooseLowPtElectron" );
  produces<pat::ElectronRefVector>("LooseLowPtElectronRef");
}
...
bool LowPtElectronFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(pat::ElectronCollection::const_iterator iele = electrons->begin() ; iele !=electrons->end(); ++iele)
    { 
	  ...
	  if((iele->pt()>1)&&(iele->electronID("ID")>7))
  ...
  iEvent.put(move(passedelectrons), "MiniLooseLowPtElectron");
  iEvent.put(move(passedelectronRef),"LooseLowPtElectronRef");
  return true;
  //else return false;
}
