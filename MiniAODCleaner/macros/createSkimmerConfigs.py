...
#print pufile
if Sample_Name == "QCD":
    SampleName = "QCD_Pt_BINNED_TuneCP5_13TeV_pythia8_RunIISummer20UL16MiniAODAPVv2-106X_mcRun2_asymptotic_preVFP_v11-v1"
    JetBINS = ['15to30','30to50','50to80', '80to120', '120to170','170to300','300to470','470to600','600to800','800to1000','1000to1400','1400to1800','1800to2400','2400to3200','3200toInf']
...
configDir = "/uscms_data/d3/nbower/FSU/Updated_LowPTIDStudies/TauCleaning/CMSSW_10_6_26/src/MiniAODSkimmer/MiniAODCleaner/configs/"+SampleName+"_LooseLowPT_Drp1/"
checkAndMakeDir(configDir)
clearDir(configDir)


TopDir = "/store/user/nbower/Events/"+SampleName+"_LooseLowPT_Drp1/"
checkAndMakeDir("/eos/uscms/"+TopDir)
...
for bin in JetBINS:
    localdir = "/store/user/nbower/Events/"+SampleName.replace("BINNED",bin)
    outputDir =TopDir+bin+"/"
    checkAndMakeDir("/eos/uscms/"+outputDir)
    InFileDir=localdir
    for file in os.listdir("/eos/uscms/"+InFileDir):
        cfg=open(configDir+file.replace(".root","cfg.py"),"w")
        print (configDir+file.replace(".root","cfg.py"),"w")
        cfg.writelines("""
...
if runSignal:
    readFiles.extend([
       ...
       'root://cmseos.fnal.gov/"""+InFileDir+"/" +file+"""'
    ])
else:
    readFiles.extend([
        #'file:patMiniAOD_standard.root'
...
if runSignal:
    process.output.fileName = 'root://cmseos.fnal.gov/"""+outputDir+"/"+file.replace(".root","_SKIMMEDLowPT.root")+"""'
    if reclusterJets:
        process.output.fileName = 'root://cmseos.fnal.gov/"""+outputDir+"/"+file.replace(".root","_SKIMMEDLowPT.root")+"""'
else:
    process.output.fileName = 'root://cmseos.fnal.gov/"""+outputDir+"/"+file.replace(".root","_SKIMMEDLowPT.root")+"""'
