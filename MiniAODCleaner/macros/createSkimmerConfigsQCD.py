...
#masses = ['15to30','30to50','50to80', '80to120', '120to170','170to300']
masses = ['300to470','470to600','600to800','800to1000']
#masses = ['1000to1400','1400to1800','1800to2400','2400to3200','3200toInf']
...
configDir = "/uscms_data/d3/nbower/FSU/Updated_LowPTIDStudies/TauCleaning/CMSSW_10_6_26/src/MiniAODSkimmer/MiniAODCleaner/configs/QCD_LowPT_step2/"
checkAndMakeDir(configDir)
clearDir(configDir)


TopDir = "/store/user/nbower/Events/"+topFolder+"_SKIMMED_LowPT/"
...
for mass in masses:
    ...
    for nf in range(1, len(files)+1):
        cfg=open(configDir+"QCD_Pt_"+mass+"_TuneCP5_13TeV_pythia8_RunIISummer20UL16MiniAODAPVv2-106X_mcRun2_asymptotic_preVFP_v11-v1_"+str(nf)+"_cfg.py","w")
        cfg.writelines("""
...
if runSignal:
    process.output.fileName = 'root://cmseos.fnal.gov/"""+outputDir+"/"+files[nf-1].replace(".root","_SKIMMED_LowPT.root")+"""'
    if reclusterJets:
        process.output.fileName = 'root://cmseos.fnal.gov/"""+outputDir+"/"+files[nf-1].replace(".root","_SKIMMED_LowPT.root")+"""'
else:
    process.output.fileName = 'root://cmseos.fnal.gov/"""+outputDir+"/"+files[nf-1].replace(".root","_SKIMMED_LowPT.root")+"""'
